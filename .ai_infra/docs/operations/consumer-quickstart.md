...
> **Trae (parallel IDE):** no `/add-plugin`. Use `python3 -m cursor_workflow activate --directory . --profile dual_ide` and follow [trae-consumer-quickstart.md](trae-consumer-quickstart.md). Default activate profile is **`with_mcp`** (Cursor only); Trae requires explicit **`dual_ide`**.
