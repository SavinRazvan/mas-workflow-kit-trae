...
When activate uses `--profile dual_ide`, both `.cursor/` (SSOT) and generated `.trae/` exist. **Shared runtime:** `.local/` trackers and `workflow-artifacts/` — both IDEs read the same files.
