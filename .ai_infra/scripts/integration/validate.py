...
from ide_contract_paths import CURSOR, TRAE, agents_dir as contract_agents_dir  # noqa: E402
...
def _paths(root: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "registry_example": root / ".cursor" / "mcp.registry.yaml.example",
        "agents_dir": contract_agents_dir(root, CURSOR),
        "trae_agents_dir": contract_agents_dir(root, TRAE),
        "trae_registry_example": root / ".trae" / "mcp.registry.yaml.example",
...
def _check_registry_file(paths: dict[str, Path]) -> CheckResult:
    ...
    trae_registry = paths["trae_registry_example"]
    trae_agents = paths["trae_agents_dir"]
    if trae_registry.is_file() and trae_agents.is_dir():
        violations.extend(
            check_registry_parity(
                agents_dir=trae_agents,
                registry_path=trae_registry,
            )
        )
    return CheckResult(
        check_id="INT-002",
...
def _check_int008(paths: dict[str, Path]) -> CheckResult:
    ...
    if not manifest_path.is_file():
        ...
        profile = (data or {}).get("profiles", {}).get("default", {})
        copy_ai = profile.get("copy_ai_infra") or []
        copy_dirs = profile.get("copy_dirs") or []
        ...
        has_cursor = any(
            isinstance(entry, dict) and entry.get("src") == ".cursor"
            for entry in copy_dirs
        )
        if not has_cursor:
            violations.append("manifest missing .cursor in copy_dirs")
    return CheckResult(
        check_id="INT-008",
        severity=Severity.P1,
        passed=not violations,
        detail="; ".join(violations) if violations else "manifest includes agent-integration + .cursor",
    )
