...
Role: Verify generated .trae/ contract plane matches .cursor/ SSOT (ADR-008).
Used By:
 - verify_all.py
        ...
        for rel in ("rules", "skills", "agents", "mcp.json"):
            ...
            if src.is_file() and dst.is_file():
                if src.read_bytes() != dst.read_bytes():
                    errors.append(f"regen drift file: .trae/{rel}")
                ...
                for path in sorted(src.rglob("*")):
                    if path.is_file():
                        ...
                        if not other.is_file() or path.read_bytes() != other.read_bytes():
                            errors.append(f"regen drift: .trae/{rel}/{rel_path}")
                            break
