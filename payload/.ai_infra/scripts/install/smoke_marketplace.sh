...
# Depends On:
#  - trae_workflow install / activate
...
"$SMOKE/.venv/bin/python" -m trae_workflow gates --directory "$SMOKE"
...
"$PY" "$KIT/payload/trae_workflow" activate \
  --directory "$PLUGIN_TARGET" \
  --source "$KIT/payload"
"$PLUGIN_TARGET/.venv/bin/python" -m trae_workflow gates --directory "$PLUGIN_TARGET"
