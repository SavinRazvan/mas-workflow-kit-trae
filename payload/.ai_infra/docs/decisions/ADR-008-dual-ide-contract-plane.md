...
Trae uses `.trae/rules/*.md`, `.trae/skills/`, and `.trae/mcp.json` (not `.cursor/` paths). Trae contract plane is generated per ADR-008 (`sync_trae_contract`); kit-dev and default consumers ship `.trae/` from activate.
...
4. **Trae distribution:** `payload/.trae/` copied on activate with `--profile default`.
5. **Shared planes:** infrastructure (`.ai_infra/`, `trae_workflow/`) and runtime (`.local/`) are IDE-agnostic.
...
- Renaming `trae_workflow` package in v1.
...
- New manifest profile `default` extends `with_mcp` with `.trae` copy_dirs.
- `install-contract.json` gains `default` required_paths under `.trae/`.
- `plane_status.py` reports `trae_contract` when profile is `default`.
