...
Agent-requested rules: `.trae/rules/agent-*.md` — follow when invoking each kit agent.
...
`GATES` in `.ai_infra/scripts/pr/prepare.py` — kit-dev runs **4** gates (testing artifacts, pytest, drift, doc facts) for PR merge prep. Maintainer hygiene uses **`make gates`** / **`trae-workflow gates`** (**6–7** steps on kit-dev with pyright + trae parity; see [gate-matrix.md](.ai_infra/docs/operations/gate-matrix.md)). Say *prepare gates green* in chat; do not paste full gate lists.
