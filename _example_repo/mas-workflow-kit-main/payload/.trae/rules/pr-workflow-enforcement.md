...
- **Architecture-impacting** changes: before prepare/merge, produce `.local/workflow-artifacts/alignment/alignment-audit.md` and `.local/workflow-artifacts/alignment/alignment-todos.md` using **`enterprise-auditor`** + `.cursor/skills/enterprise-architecture-audit/SKILL.md` (focused alignment pass when a full scorecard is not required; see [advisory-audit-alignment-enforcement.mdc](advisory-audit-alignment-enforcement.mdc)).
